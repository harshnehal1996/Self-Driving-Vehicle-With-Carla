...
2. Only project to the cameraframes that are nearby to the points.
In order to achieve the latter, before projecting a lidar point, first the nearest cameraframe(**K**) to the point is searched in the linked-list. Then the point is only projected to the cameraframes that are less than 40m away from "K". 
...
To improve accuracy further I considered two other scenarios.
