...
class trainUtils(object):
    ...
    def train_broken_trajectory(self, env, expert, actor, critic, discriminator, critic_optimizer, discriminator_optimizer,                                max_expert_trajectory_length=32, max_policy_trajectory_length=32, max_attempt=3,                                max_gradient_steps=1, expert_save=True, normalize=True, game=[]):
        ...
        while gradient_steps < max_gradient_steps and not done:
            ...
            print('policy_improvement', policy_improvement,'discriminator_loss', discriminator_loss, 'value_loss', value_loss)
...
out = config.grid_dir
ds = Dataset(dumps, carla_map, 1024, 1024, out)
...
env = Environment(3, ds, client, world)
env.initialize()
actor = Actor()
disc = Discriminator()
discriminator_optimizer = optim.Adam(disc.parameters(), lr=0.0001, betas=(0.5, 0.999))
critic = Critic()
...
for i in range(config.num_epoch):
    tr.train_broken_trajectory(env, ds, actor, critic, disc, critic_optimizer, discriminator_optimizer)
    if (i+1) % config.save_every == 0:
        torch.save({
                ...
                'c_opt_state_dict' : critic_optimizer.state_dict(),
                'd_opt_state_dict' : discriminator_optimizer.state_dict(),
                }, config.path_to_save + 'gail_checkpoint_' + str(i) + '.pth')
