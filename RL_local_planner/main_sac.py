...
class Critic(nn.Module):
    ...
    def forward(self, x_static, index_seq, sequence, input_lengths, mask, action):
        ...
        clipped_action = torch.stack([torch.clip(action[:,1], min=0), -torch.clip(action[:,1], max=0)], axis=-1)
...
class Actor(nn.Module):
    ...
    def sample_from_density_fn(self, mu, log_sigma, mask, return_pi=True, deterministic=False):
        if deterministic:
            ...
            if return_pi:
                pi = ndist.log_prob(sample) - (2 * (np.log(2) - sample - F.softplus(-2 * sample)))
            else:
                pi = None
        
        action = torch.tanh(sample)
        throttle = action[:,1]
        throttle = mask * (throttle * self.continious_range_1 + self.offset_1) + (1 - mask) * (throttle * self.continious_range_2 + self.offset_2)
        action = torch.stack([action[:,0] * config.continious_range, throttle], axis=1)
...
class Experience_Buffer(object):
    ...
    def generate_batch(self, batch_size, ratio=1, w=1, cmin=15000):
        ...
        mask_t = torch.LongTensor([self.state_buffer[i][3] for i in idx]).to(device)
        input_lengths_t = np.array([len(index_t[i]) for i in range(len(index_t))], dtype=np.int32)
        index_t = nn.utils.rnn.pad_sequence(index_t)
        batch_t = nn.utils.rnn.pad_sequence(batch_t)
        ...
        mask_t1 = torch.LongTensor([self.next_state_buffer[i][3] for i in idx]).to(device)
        ...
        batch = { 'state' : (x_static_t, index_t, batch_t, input_lengths_t, mask_t), 'action' : torch.Tensor(self.action_buffer[idx]).to(device), 'reward' : torch.Tensor(self.reward_buffer[idx]).to(device), 'next_state' : (x_static_t1, index_t1, batch_t1, input_lengths_t1, mask_t1),  'done' : torch.Tensor(self.done_buffer[idx]).to(device)}
...
def learn(env, buffer, writer, q1, q2, q1_target, q2_target, actor, actor_optimizer, q1_optimizer, q2_optimizer, loss_fn, max_trajectory_length=500, num_policy_trajectory=3, max_attempt=3, random_explore=False, counter=0, train_steps=0, neta=1):
    ...
    def get_data(this_batch):
        ...
        batch_t = [this_batch[i][2].to(device) for i in range(len(this_batch))]
        input_lengths_t = np.array([len(index_t[i]) for i in range(len(index_t))], dtype=np.int32)
        mask_t = torch.LongTensor([this_batch[i][3] for i in range(len(this_batch))]).to(device)
        index_t = nn.utils.rnn.pad_sequence(index_t)
        batch_t = nn.utils.rnn.pad_sequence(batch_t)
    ...
    while timestep < max_trajectory_length and count < batch_size:
        ...
        for i in range(batch_size):
            if state[i][10] == 0:
                ...
                this_batch.append([torch.from_numpy(state[i][1]), state[i][3][0], state[i][3][1], state[i][7]])
                index.append(i)

        with torch.no_grad():
            if len(index):
                if random_explore:
                    x1 = np.random.uniform(low=-actor.continious_range_1 + actor.offset_1, high=actor.continious_range_1 + actor.offset_1, size=len(index))
                    x2 = np.random.uniform(low=-actor.continious_range_2 + actor.offset_2, high=actor.continious_range_2 + actor.offset_2, size=len(index))
                    mask = np.array([this_batch[i][3] for i in range(len(this_batch))])
                    action = np.stack([np.random.uniform(low=-config.continious_range, high=config.continious_range, size=len(index)), mask * x1 + (1 - mask) * x2], axis=1).astype(np.float32)
            ...
            for i in range(batch_size):
                ...
                if start_state[i] != 2 and state[i][10] != 1:
                    ...
                    buffer.add_observation(deepcopy(policy_data[i]), deepcopy(batch_action[i][:-1]), rewards[i], [torch.from_numpy(state[i][1]).clone(), state[i][3][0].clone(), state[i][3][1].clone(), state[i][7]], done)
        ...
        if counter >= config.update_every and buffer.size > config.min_buffer_size and not random_explore:
            ...
            counter = 0
            ac_loss, q1_loss, q2_loss, entropy_1, entropy_2 = train_from_buffer(buffer, writer, q1, q2, q1_target, q2_target, actor, actor_optimizer, q1_optimizer, q2_optimizer, loss_fn, num_steps=num_batches, train_steps=train_steps, neta=neta)
    ...
    ret = {'num_states' : num_states, 'train_stats' : [counter, train_steps, ac_loss, q1_loss, q2_loss, entropy_1 + entropy_2], 'total_rewards' : total_rewards, 'trajectory_length' : trajectory_length}
...
def train_from_buffer(buffer, writer, q1, q2, q1_target, q2_target, actor, actor_optimizer, q1_optimizer, q2_optimizer, loss_fn, qnormclip=1, num_steps=1, train_steps=0, neta=0.996, print_steps=False):
    ...
    for i in range(num_steps):
        ...
        config.alpha[0,0] = np.clip(config.alpha[0,0].item() - config.lr * g_alpha_1.item(), config.min_alpha[0], config.max_alpha[0])
        config.alpha[0,1] = np.clip(config.alpha[0,1].item() - config.lr * g_alpha_2.item(), config.min_alpha[1], config.max_alpha[1])
        ...
        writer.add_scalar('entropy_1', differential_entropy_1.item(), train_steps)
        writer.add_scalar('entropy_2', differential_entropy_2.item(), train_steps)
        
        if print_steps:
            print('step[%d/%d] : actor %.4f q1 %.4f q2 %.4f steer entropy %.4f throttle entropy %.4f' % (i+1, num_steps, ac_loss, q1_loss, q2_loss, differential_entropy_1.item(), differential_entropy_2.item()))
    
    print('step %d  actor %.4f  q1 %.4f  q2 %.4f  entropy=(%.4f, %.4f)  alpha=(%.4f, %.4f)' % (num_steps, sum_ac_loss / num_steps, sum_q1_loss / num_steps, sum_q2_loss / num_steps, sum_entropy_1 / num_steps, sum_entropy_2 / num_steps, config.alpha[0,0].item(), config.alpha[0,1].item()))
