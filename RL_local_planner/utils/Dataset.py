...
from utils.mapGenerator import MapGenerator
...
class Dataset(object):
    def __init__(self, dumps, carla_map, size_x, size_y, occupancy_path, grid=[], map_ratio=4, window=3, out_patience=2, speed_on_stop_th=0.5):
        ...
        self.orientations = [dumps[i]['orientation']
        ...
        self.df_trajectory = [[None for _ in range(
            self.num_heros[j])] for j in range(len(dumps))]
    ...
    def __load_occupancy_grid(self, occupancy_path):
        ...
        drivable[np.logical_or(np.logical_or(
            self.grid == 50, self.grid == 187), self.grid == 185)] = 255
        lanes = np.zeros(self.grid.shape, dtype=np.uint8)
        lanes[np.logical_or(self.grid == 187, self.grid == 185)] = 255
        comp = 12 / self.map_ratio
        self.sidewalk = torch.from_numpy(cv2.resize(sidewalk, (int(
            self.grid.shape[0] / comp), int(self.grid.shape[0] / comp))).astype(np.float32) / 255)
        self.drivable = torch.from_numpy(cv2.resize(drivable, (int(
            self.grid.shape[0] / comp), int(self.grid.shape[0] / comp))).astype(np.float32) / 255)
        self.lanes = torch.from_numpy(cv2.resize(lanes, (int(
            self.grid.shape[0] / comp), int(self.grid.shape[0] / comp))).astype(np.float32) / 255)
        self.sidewalk = F.pad(self.sidewalk, (self.h_cache_size_x,
                              self.h_cache_size_x, self.h_cache_size_y, self.h_cache_size_y))
        self.drivable = F.pad(self.drivable, (self.h_cache_size_x,
                              self.h_cache_size_x, self.h_cache_size_y, self.h_cache_size_y))
        self.lanes = F.pad(self.lanes, (self.h_cache_size_x,
                           self.h_cache_size_x, self.h_cache_size_y, self.h_cache_size_y))
        self.corrected_map_ratio = 12 / \
            (self.grid.shape[0] / (self.grid.shape[0] // comp))
    ...
    def __load_in_pd(self, dumps):
        for d in range(len(dumps)):
            ...
            for j in range(self.num_heros[d]):
                ...
                df_ins = pd.DataFrame(stacked_traj, columns=[
                                      'trajectory', 'time', 'throttle', 'steering', 'brake', 'v_x', 'v_y', 'a_x', 'a_y'])
    ...
    def __create_cached_map(self, state):
        cached_map = torch.Tensor(
            config.conv_size[0], config.cache_size_y, config.cache_size_x)
        static_map = state[0]
        img = static_map[0]
        stop_target, through_target = static_map[-1]
        occ_pix = self.world_to_pixel(
            state[9], offset=(-self.h_cache_size_x, -self.h_cache_size_y))
        s_pt = np.around(state[4] * self.map_ratio).astype(np.int32)
        reward_field_x, reward_field_y = static_map[1]
        cached_map[0] = torch.from_numpy(reward_field_x[s_pt[1]-self.h_cache_size_y: s_pt[1] +
                                         self.h_cache_size_y+1, s_pt[0]-self.h_cache_size_x: s_pt[0]+self.h_cache_size_x+1].toarray())
        cached_map[1] = torch.from_numpy(reward_field_y[s_pt[1]-self.h_cache_size_y: s_pt[1] +
                                         self.h_cache_size_y+1, s_pt[0]-self.h_cache_size_x: s_pt[0]+self.h_cache_size_x+1].toarray())
        cached_map[2] = torch.from_numpy(img[s_pt[1]-self.h_cache_size_y: s_pt[1]+self.h_cache_size_y+1,
                                         s_pt[0]-self.h_cache_size_x: s_pt[0]+self.h_cache_size_x+1].toarray())
        cached_map[3] = self.drivable[occ_pix[1]-self.h_cache_size_y: occ_pix[1] +
                                      self.h_cache_size_y+1, occ_pix[0]-self.h_cache_size_x: occ_pix[0]+self.h_cache_size_x+1]
        ...
        cached_map[4] = self.lanes[occ_pix[1]-self.h_cache_size_y: occ_pix[1] +
                                   self.h_cache_size_y+1, occ_pix[0]-self.h_cache_size_x: occ_pix[0]+self.h_cache_size_x+1]
        cached_map[5] = torch.from_numpy(stop_target[s_pt[1]-self.h_cache_size_y: s_pt[1] +
                                         self.h_cache_size_y+1, s_pt[0]-self.h_cache_size_x: s_pt[0]+self.h_cache_size_x+1].toarray())
        cached_map[6] = torch.from_numpy(through_target[s_pt[1]-self.h_cache_size_y: s_pt[1] +
                                         self.h_cache_size_y+1, s_pt[0]-self.h_cache_size_x: s_pt[0]+self.h_cache_size_x+1].toarray())
        cached_map[7: 7 + config.embedding_size] = 0
        cached_map[7 + config.embedding_size:] = 0
        offset = s_pt.copy() - \
            np.array([self.h_cache_size_x, self.h_cache_size_y])
        state[2] = [cached_map, offset]

    def world_to_pixel(self, location, offset=(0, 0)):
        x = self.scale * self.corrected_map_ratio * \
            (location[0] - self._world_offset[0])
        y = self.scale * self.corrected_map_ratio * \
            (location[1] - self._world_offset[1])
    ...
    def __append_dynamic_features(self, idx, sim, obs_index, state, pos, dynamic_objects):
        ...
        rnn_features = np.zeros(
            (config.lookback, len(dynamic_objects), 4), dtype=np.float32)
    ...
    def extract_trajectory(self, sim=-1, hero=-1, trajectory_no=-1):
        try:
            ...
            df = self.df_trajectory[sim][hero][self.df_trajectory[sim]
                                               [hero]['trajectory'] == trajectory_no]
            index = list(df['idx'])
            ...
            state = self.mgen.sparse_build_multiple_area_upwards(path, max_reward=1.0, min_reward=0.3,
                                                                 zero_padding=max(
                                                                     self.h_cache_size_y, self.h_cache_size_x) + 1,
            ...
            if len(state[-2]) != len(path):
                ...
                qpoints = self.fix_end_line(
                    state[1], state[0], e_pt, offset=-5*self.window, size=7*self.window)
                mandate = False
                if not len(qpoints):
                    if len(road) > 1:
                        _last = -2
                        ...
                        _last = -1
                    e_pt = np.array(
                        [road[_last].transform.location.x, road[_last].transform.location.y])
                    e_pt = np.around((e_pt - origin) *
                                     self.map_ratio).astype(np.int32)
                    qpoints = self.fix_end_line(
                        state[1], state[0], e_pt, offset=-6*self.window, size=7*self.window)
                    if not len(qpoints):
                ...
                if mandate or end_speed > self.speed_on_stop_th:
                    self.color_points_in_quadrilateral(
                        qpoints, target_pass, val=1)
                    quit_stat = False
                else:
                    self.color_points_in_quadrilateral(
                        qpoints, target_stop, val=1)
                    quit_stat = True
                ...
                while i < len(index):
                    ...
                    y = position[5]
                    _j, _i = round(
                        self.map_ratio * (x - origin[0])), round(self.map_ratio * (y - origin[1]))
                    val = state[0][_i - w: _i + w +
                                   1, _j - w: _j + w + 1].sum()
                    if val > 0:
                        break
                    i += 1
                ...
                trajectory_list = [state, [], [], angle, s_pt, e_pt, i, origin, [], [
                    x, y], [], [road_segment, len(path), quit_stat], 0, sim, hero, trajectory_no]
    ...
    def trajectory_step(self, state):
        ...
        d = np.linalg.norm(F)
        if d < 1e-4:
            print('track out of range hero=%d, trajectory=%d, index=%d' %
                  (hero, trajectory_no, step_index))
            F = angle.copy()
        ...
        c_x, c_y = round(
            self.radius * F[0] + loc_x), round(self.radius * F[1] + loc_y)
        ...
        state[1] = cached_map[:, c_y-self.h_dynamic_size_y: c_y+self.h_dynamic_size_y +
                              1, c_x-self.h_dynamic_size_x: c_x+self.h_dynamic_size_x+1]
        dynamic_map = state[1]
        hero_start = 7 + config.embedding_size
        val = torch.Tensor([v_x / config.v_scale, v_y / config.v_scale,
                           longitudinal_acc / config.a_scale, steering_angle]).reshape(-1, 1)
        ...
        for i in range(self.num_actor[sim]):
            ...
            x, y = round(
                (obs[4] - origin[0]) * self.map_ratio), round((obs[5] - origin[1]) * self.map_ratio)
            _yaw_ = obs[1]
            if i == hero:
                continue
            ...
            if abs(x - c_x - offset[0]) < self.h_dynamic_size_x and abs(y - c_y - offset[1]) < self.h_dynamic_size_y:
                if self.isVehicle[sim][i]:
                    v_pos.append([np.cos(_yaw_ * np.pi / 180), np.sin(_yaw_ * np.pi / 180), x - c_x -
                                 offset[0] + self.h_dynamic_size_x, y - c_y - offset[1] + self.h_dynamic_size_y])
                    v_dynamic_objects.append(i)
                else:
                    p_pos.append([np.cos(_yaw_ * np.pi / 180), np.sin(_yaw_ * np.pi / 180), x - c_x -
                                 offset[0] + self.h_dynamic_size_x, y - c_y - offset[1] + self.h_dynamic_size_y])
                    p_dynamic_objects.append(i)

        state[10] = []
        self.__append_dynamic_features(
            0, sim, obs_index, state, v_pos, v_dynamic_objects)
        self.__append_dynamic_features(
            1, sim, obs_index, state, p_pos, p_dynamic_objects)

        if len(state[10][0][0]) and len(state[10][1][0]):
            accum_tensor = torch.cat(
                [state[10][0][0], state[10][1][0]], axis=1)
...
class Buffer(object):
    ...
    def sample_data_points(self, num_points, shuffle=True):
        ...
        while dpts < num_points:
            ...
            while True:
                try:
                    control, done = self.data_generator.trajectory_step(
                        state[0])
                except:
                    traceback.print_exception(*sys.exc_info())
                    return
                ...
                data.append([[deepcopy(state[0][10]), state[0][1].clone(
                ), quit], control, (state[0][-3], state[0][-2], state[0][-1], state[0][6])])
                dpts += 1
        ...
        print('updated train_size %d, val_size %d' %
              (self.train_size, len(self.val_data)))
